...
typedef enum {
    ...
    // --- 32 位视图 W0-W30（X 寄存器的低 32 位）---
    // 反汇编里出现 W 寄存器时用它读取，语义与 X 的低半部分一致。
    VC_REG_W0,  VC_REG_W1,  VC_REG_W2,  VC_REG_W3,  VC_REG_W4,  VC_REG_W5,  VC_REG_W6,  VC_REG_W7,
    VC_REG_W8,  VC_REG_W9,  VC_REG_W10,  VC_REG_W11,  VC_REG_W12,  VC_REG_W13,  VC_REG_W14,  VC_REG_W15,
    VC_REG_W16,  VC_REG_W17,  VC_REG_W18,  VC_REG_W19,  VC_REG_W20,  VC_REG_W21,  VC_REG_W22,  VC_REG_W23,
    VC_REG_W24,  VC_REG_W25,  VC_REG_W26,  VC_REG_W27,  VC_REG_W28,  VC_REG_W29,  VC_REG_W30,
...
typedef enum {
    ...
    VC_HOOK_EMU_STOP       = 1 << 22,  // 模拟正常停机（目标函数 ret 返回）回调
} vc_hook_type;
...
/*
 * vc_cb_emu_stop_t — EMU_STOP 回调签名（自定义扩展）
 *
 * 目标函数整体 ret 返回（信号循环正常结束、非错误退出）那一刻触发一次，此时寄存器已同步、
 * guest 内存都还在（identity map）。适合做收尾（如 trace_unidbg_dump 在此落盘）。
 */
typedef void (*vc_cb_emu_stop_t)(vm_context* ctx, void* user_data);
...
// 开关：trace 期间遇到 pthread_create 时，自动把新线程的 start_routine 包装进 VM 追踪
// （新线程在 VM 内执行并产生独立 per-thread trace 文件）。**默认关**，按需开：
// trace(f, path, &ctx) 拿 ctx 后本函数传 true。（trace 层特性，实现在 EastTrace.cpp。）
//
// 实现上是【开启时才挂 hook】：传 true 时才注册一个地址锁定到 pthread_create 入口的独立
// BLOCK hook（和逐指令 trace 的判定逻辑完全分离）。不开就压根没有这个 hook，零影响。
...
//  CODE 回调内的快速寄存器读取
//
//  只在 CODE 回调（VC_HOOK_CODE）内部有效——那时寄存器文件是稳定的。
//  逐指令热路径专用：一次取到寄存器文件直接指针，之后整块 memcpy / 下标读取，
//  不需要逐寄存器的函数调用，也不需要接触底层引擎句柄。
// ============================================================

/*
 * vc_code_fast_get_xregs — 返回指向 X0-X31 寄存器文件的直接指针
 *
 * 返回数组 [0..30]=X0-X30，[31]=SP。取一次指针缓存起来，之后逐指令直接下标读取
 * 或整块 memcpy 做快照。仅 CODE 回调内有效；非回调期或上下文无效时返回 NULL。
 */
const uint64_t* vc_code_fast_get_xregs(vm_context* ctx);

/*
 * vc_code_fast_get_nzcv — 返回当前指令的条件标志寄存器 NZCV
 *
 * NZCV 由引擎按需组装（无稳定指针），所以取值而非取指针。仅 CODE 回调内有效。
 */
uint32_t vc_code_fast_get_nzcv(vm_context* ctx);

/*
 * vc_reg_read_fast — CODE 回调内快速读取单个寄存器
 *
 * 参数同 vc_reg_read，但走 fast 路径（仅 CODE 回调内有效）。适合按需读取 X 之外的
 * 寄存器（Q0-Q31 SIMD、W0-W30 等）——大多数指令不碰它们，用到时才读。
 */
vc_err vc_reg_read_fast(vm_context* ctx, vc_reg reg, void* value);

/*
 * vc_maps_generation — 返回 VCPU 的 /proc/self/maps 缓存代号（generation）
 *
 * VCPU 每次刷新 maps 缓存（拦到 mmap/mprotect/munmap 后 + 缺页兜底）就 +1。trace 层用它
 * 判断"内存布局是否变过"，只在变了时才重建区域表，平时零重扫。只读，不改任何 VCPU 状态。
 */
uint32_t vc_maps_generation(void);
 ...
 *   VC_HOOK_EMU_STOP                   → vc_cb_emu_stop_t
 *
 * @param ctx        VM 上下文
...
// ============================================================
//  CPU 事件回调 —— 把底层 UC_HOOK_* 包装成语义清晰的事件
//  (「有个外部函数调用」「遇到 SVC」「读/写内存」…),比裸 hook 好用。
//  set_cpu_event_callback 注册回调,vc_set_event_mask 控制关心哪些事件。
// ============================================================

// 事件类型
typedef enum {
    VC_CPU_EVENT_CODE  = 0,   // 逐指令
    VC_CPU_EVENT_BLOCK = 1,   // 基本块入口
    VC_CPU_EVENT_INTR  = 2,   // 中断/异常
    VC_CPU_EVENT_SVC   = 3,   // SVC 系统调用
    VC_CPU_EVENT_MEM_READ  = 4,
    VC_CPU_EVENT_MEM_WRITE = 5,
    VC_CPU_EVENT_EXTERNAL_JUMP = 6,   // 跳出到外部函数
} vc_cpu_event_type;

// 事件开关掩码(传给 vc_set_event_mask)
typedef enum {
    VC_EVENT_ENABLE_CODE  = 1 << 0,   // 逐指令:开销大,默认关
    VC_EVENT_ENABLE_BLOCK = 1 << 1,
    VC_EVENT_ENABLE_INTR  = 1 << 2,
    VC_EVENT_ENABLE_SVC   = 1 << 3,
    VC_EVENT_ENABLE_MEM   = 1 << 4,   // 内存读写:开销大,默认关
    VC_EVENT_ENABLE_EXTERNAL_JUMP = 1 << 5,
} vc_event_mask;

// 默认掩码:BLOCK | INTR | SVC | EXTERNAL_JUMP(CODE/MEM 默认关)
...
// 事件信息(按 type 取用相应字段)
typedef struct {
    vc_cpu_event_type type;
    uint64_t address;
    uint32_t size;
    uint32_t intno;
    uint32_t insn_bytes;
    uint64_t syscall_nr;
    const char* symbol_name;   // EXTERNAL_JUMP 时为目标符号名
    uint64_t mem_value;        // MEM_WRITE 时为写入值
    vc_event_action action;    // 回调可写:SKIP 跳过本次
} vc_cpu_event_info;
...
/*
 * vc_set_event_mask — 设置关心哪些事件(位或 VC_EVENT_ENABLE_*)。
 *   开 CODE / MEM 会显著变慢(逐指令 / 逐访存回调)。
 */
void vc_set_event_mask(vm_context* ctx, uint32_t mask);

/*
 * set_cpu_event_callback — 注册 CPU 事件回调。
 *   callback 传 NULL 取消。默认掩码见 VC_EVENT_DEFAULT_MASK。
 */
void set_cpu_event_callback(vm_context* ctx,
                            vc_cpu_event_callback callback,
                            void* user_data);
