...
/*
  Callback function for high-performance tracing of AArch64 code.
  ...
  Notes:
  - This callback is invoked synchronously before the current instruction
    executes.
  - Calling uc_emu_stop() from this callback stops emulation immediately.
  - uc_mem_read() from this callback observes memory before the instruction
    executes.
  - Read registers on demand with uc_reg_read_fast() or
    uc_reg_read_fast_batch().
  - Modify registers with uc_reg_write_fast().
  - This hook is currently supported on AArch64 only.
 */
typedef void (*uc_cb_hookcode_fast_t)(uc_engine *uc, uint64_t address,
...
typedef enum uc_hook_type {
    ...
    // Hook code with synchronous AArch64 pre-execution callbacks for
    // high-performance monitoring.
    // This hook is currently supported on AArch64 only.
    UC_HOOK_CODE_FAST = 1 << 18,
    // Hook signal requests from uc_signal_request().
    // Callback is invoked after emulation stops but before uc_emu_start returns.
    UC_HOOK_SIGNAL = 1 << 19,
} uc_hook_type;
...
/*
  Callback function for signal events (UC_HOOK_SIGNAL).
  Invoked after emulation stops due to uc_signal_request(), before
  uc_emu_start() returns. The callback may set PC and registers to redirect
  execution (e.g., to a signal handler).

  @signo: the signal number passed to uc_signal_request()
  @user_data: user data passed to uc_hook_add()

  @return: return true if the signal was handled (caller should resume
  emulation), or false to stop (uc_emu_start returns normally).
*/
...
typedef enum uc_control_type {
    ...
    // Write: @args = (int)
    UC_CTL_TLB_TYPE,
    // Enable automatic host-pointer page mapping for unmapped accesses.
    //
    // When enabled, Unicorn may automatically map the current host page with
    // uc_mem_map_ptr() when an unmapped access happens and there is no matching
    // UC_HOOK_MEM_*_UNMAPPED hook for that access.
    //
    // This fork now only auto-maps addresses that fall inside explicit
    // guest->host ranges registered with uc_host_ptr_auto_map_add_range() or
    // uc_vm_image_add_host_ptr_auto_range(). Addresses outside those ranges
    // continue through Unicorn's normal *_UNMAPPED error path.
    //
    // Current implementation notes:
    // - private Unicorn fork feature, not upstream
    // - intended for host=arm64 / guest=arm64 style direct host-pointer usage
    // - explicit uc_mem_map()/uc_mem_map_ptr() mappings still take priority
    // - the auto-map uses the current Unicorn target page size
    // - the inserted page keeps the permissions declared on the matching range
    //
    // Write: @args = (int)
    // Read:  @args = (int*)
    UC_CTL_HOST_PTR_AUTO_MAP,
...
#define uc_ctl_get_host_ptr_auto_map(uc, enabled)                              \
    uc_ctl(uc, UC_CTL_READ(UC_CTL_HOST_PTR_AUTO_MAP, 1), (enabled))
#define uc_ctl_set_host_ptr_auto_map(uc, enabled)                              \
    uc_ctl(uc, UC_CTL_WRITE(UC_CTL_HOST_PTR_AUTO_MAP, 1), (enabled))
...
// Shared VM image/template used to create multiple Unicorn contexts that
// reuse the same host-oriented memory layout description.
struct uc_vm_image;
...
/*
 Close a Unicorn engine instance.
  ...
  If @uc was created by uc_open_from_image() and the source image has a
  non-zero internal pool capacity, uc_close() may recycle the engine into the
  image's idle cache instead of destroying it immediately. The handle is still
  invalid to the caller after uc_close() returns.
...
/*
 Create a reusable VM image/template.

 The image stores a host-oriented memory layout template plus selected engine
 control values. It can later be instantiated into multiple Unicorn contexts
 with uc_open_from_image().

 This fork-oriented API is designed for host-backed ARM64 style workloads where
 many worker contexts execute the same code/data layout concurrently.

 Notes:
 - Hooks are NOT shared. Register hooks per instantiated context.
 - uc_mem_map_ptr()-backed regions share the caller-provided host backing.
 - uc_mem_map()-backed regions are replayed per context and remain private.
 - Idle contexts can optionally be cached per image and reused later.
 - This API still does NOT make TB/TLB state globally shared across all live
   contexts at the same time.

 @arch: architecture type (UC_ARCH_*)
 @mode: hardware mode. This is combined of UC_MODE_*
 @image: pointer that receives the created image handle
...
/*
 Close a VM image/template created by uc_vm_image_open().

 @image: image handle returned by uc_vm_image_open()
 */
UNICORN_EXPORT
void uc_vm_image_close(uc_vm_image *image);

/*
 Add a private RAM mapping template to a VM image.

 Each context created from this image will create its own RAM region for this
 range, similar to calling uc_mem_map() on each context.
 */
UNICORN_EXPORT
...
/*
 Add a host-backed RAM mapping template to a VM image.

 Each context created from this image will replay uc_mem_map_ptr() with the same
 guest range, permissions and host pointer.
 */
UNICORN_EXPORT
...
/*
 Configure a CPU model that will be applied before uc_open_from_image()
 initializes the Unicorn context.
 */
UNICORN_EXPORT
uc_err uc_vm_image_set_cpu_model(uc_vm_image *image, int model);

/*
 Configure a target page size that will be applied before uc_open_from_image()
 initializes the Unicorn context.
 */
UNICORN_EXPORT
...
/*
 Configure the TCG translation buffer size that will be applied before
 uc_open_from_image() initializes the Unicorn context.
 */
UNICORN_EXPORT
uc_err uc_vm_image_set_tcg_buffer_size(uc_vm_image *image, uint32_t size);

/*
 Configure the TLB mode applied after the Unicorn context has been created from
 this image.
 */
UNICORN_EXPORT
...
/*
 Configure host-pointer auto-map behavior applied after the Unicorn context has
 been created from this image.
 */
UNICORN_EXPORT
uc_err uc_vm_image_set_host_ptr_auto_map(uc_vm_image *image, bool enabled);

/*
 Register an explicit guest->host range that is eligible for host-pointer
 auto-map when this image creates contexts.

 Notes:
 - this does NOT create an immediate mapping on its own
 - it only authorizes future automatic page mapping for unmapped accesses
 - guest_base, size and host_base should be aligned to the image page size
   when one has been configured
 - the declared perms are applied to auto-mapped pages and therefore control
   whether later accesses become *_PROT or succeed
 */
UNICORN_EXPORT
...
/*
 Configure the maximum number of idle Unicorn contexts cached inside this image.

 When capacity is non-zero, contexts created by uc_open_from_image() may be
 recycled by uc_close() and later reused by a future uc_open_from_image() call.
 Recycled contexts keep their internal TB/JIT cache, while CPU state, memory
 state, exits and hook registrations are reset back to the image baseline
 before they become idle again.

 Notes:
 - Call this during image setup, before worker threads start using the image.
 - A capacity of 0 disables the idle context pool.
 - This pool does NOT make a single uc_engine usable by multiple threads at the
   same time; each uc_open_from_image() still returns a private context.
 */
UNICORN_EXPORT
uc_err uc_vm_image_set_pool_capacity(uc_vm_image *image, uint32_t capacity);

/*
  Add a TB prewarm address to the image.

 When uc_open_from_image() instantiates a context, each address added here is
 passed to uc_ctl_request_cache() after mappings/configuration are applied.
 This only prewarms the TB that starts at the given address, not every possible
 path in the surrounding function.
 */
UNICORN_EXPORT
uc_err uc_vm_image_add_prewarm(uc_vm_image *image, uint64_t address);

/*
 Create a Unicorn context from a VM image/template.

  The new context gets its own CPU/register/hook state, while reusing the
  image's mapping template and host-backed pointers.

  Hooks, register state, stop/quit state and callbacks remain private to the
  returned context. If the image has an internal pool capacity, this call may
  return a recycled context that has already been reset back to the image
  baseline and kept warm in the idle cache.
  */
UNICORN_EXPORT
...
/*
 Read register value.
  ...
  NOTE: uc_reg_read() reads the engine's live CPU state. In callbacks that do
  not guarantee per-instruction synchronization, such as UC_HOOK_BLOCK, the
  returned value may not correspond exactly to the callback address. Inside
  UC_HOOK_CODE_FAST callbacks, prefer uc_reg_read_fast() or
  uc_reg_read_fast_batch() for callback-local fast reads.
...
/*
 Read register value from the current UC_HOOK_CODE_FAST callback state.

  @uc: handle returned by uc_open()
  @regid:  register ID that is to be retrieved.
  @value:  pointer to a variable storing the register value.

  NOTE: This API is only valid while executing a UC_HOOK_CODE_FAST callback.
  The returned value reflects the current callback-local live CPU state before
  the instruction executes. If you mutate registers inside the callback and read
  them again, the updated values are returned. Unsupported registers, calls from
  other hook types (for example UC_HOOK_CODE, UC_HOOK_BLOCK, UC_HOOK_MEM_*), or
  calls made after the callback returns will fail with UC_ERR_ARG.

  @return UC_ERR_OK on success; UC_ERR_ARG if no current UC_HOOK_CODE_FAST
  context is available or the requested register is unsupported.
 */
/*
 Return a direct pointer to the AArch64 xregs[32] array (X0-X30, SP) from the
 current UC_HOOK_CODE_FAST callback state.

 Index mapping: [0..28]=X0-X28, [29]=X29/FP, [30]=X30/LR, [31]=SP.
 NZCV is NOT in this array; use uc_code_fast_get_nzcv() for that.

 @return pointer to xregs[32], or NULL if not in a CODE_FAST callback.
 */
UNICORN_EXPORT
const uint64_t* uc_code_fast_get_xregs(uc_engine *uc);

/*
 Return the NZCV flags from the current UC_HOOK_CODE_FAST callback state.

 @return NZCV value (bits 31-28), or 0 if not in a CODE_FAST callback.
 */
UNICORN_EXPORT
uint32_t uc_code_fast_get_nzcv(uc_engine *uc);

/*
 General-purpose versions — work in ANY callback (block, code, mem, interrupt).
 Reads live CPU state via uc->cpu->env_ptr.
 */
UNICORN_EXPORT
const uint64_t* uc_get_xregs(uc_engine *uc);

UNICORN_EXPORT
uint32_t uc_get_nzcv(uc_engine *uc);

UNICORN_EXPORT
uc_err uc_reg_read_fast(uc_engine *uc, int regid, void *value);

/*
 Read multiple register values from the current UC_HOOK_CODE_FAST callback
 state.
  ...
  @return UC_ERR_OK on success; UC_ERR_ARG on invalid arguments, unsupported
  registers, or if no current UC_HOOK_CODE_FAST context is available.
 */
UNICORN_EXPORT
...
/*
 Write a register value to the current UC_HOOK_CODE_FAST callback state.
  ...
  NOTE: This API is only valid while executing a UC_HOOK_CODE_FAST callback.
  The write affects the current live CPU state immediately. If @regid is PC,
  Unicorn exits the current TB and resumes from the new PC.
...
/*
 Request a signal to be delivered. This stops emulation and triggers
 UC_HOOK_SIGNAL callbacks before uc_emu_start() returns.
 Typically called from hook callbacks (e.g., UC_HOOK_INSN for MRS).

 @uc: handle returned by uc_open()
 @signo: signal number (e.g., SIGILL, SIGSEGV)

 @return UC_ERR_OK on success, or other value on failure (refer to uc_err enum
   for detailed error).
*/
UNICORN_EXPORT
uc_err uc_signal_request(uc_engine *uc, int signo);

/*
 Skip the current translation block without executing its translated instructions.

 Call this from a UC_HOOK_BLOCK callback after modifying PC (via uc_reg_write)
 to redirect execution. Without this call, the JIT-compiled instructions of the
 current TB would still execute with stale register values, because
 uc_reg_write(PC) only updates env->pc — it does not redirect the in-flight
 JIT code.

 Internally this sets skip_sync_pc_on_exit (so cpu_restore_state won't overwrite
 the PC you set) and triggers an exit request that check_exit_request picks up
 before any translated instruction runs. Emulation continues from the new PC;
 uc_emu_start does NOT return.

 This function returns normally — the actual TB exit happens at the next
 check_exit_request point (gen_tb_start), which is after the block hook but
 before translated instructions in this fork's TB layout.

 @uc: handle returned by uc_open()
*/
UNICORN_EXPORT
void uc_block_skip(uc_engine *uc);
...
/*
 ...
 @...: variable arguments (depending on @type)
  ...
  @uc_tcg_op_code and @uc_tcg_op_flag for details.
    NOTE: if @type = UC_HOOK_CODE_FAST, @callback must be of type
  @uc_cb_hookcode_fast_t. The callback is invoked synchronously before each
  matched AArch64 instruction. Use uc_reg_read_fast(),
  uc_reg_read_fast_batch() and uc_reg_write_fast() inside the callback for
  callback-local register access.
...
/*
 Register an explicit guest->host range for host-pointer auto-map on a live
 Unicorn context.

 Notes:
 - this does NOT create an immediate memory mapping
 - it only authorizes future automatic page mapping for unmapped accesses when
   uc_ctl_set_host_ptr_auto_map(uc, 1) is enabled
 - guest_base, size and host_base must be aligned to the current Unicorn target
   page size
 - the declared perms are applied to auto-mapped pages
 */
UNICORN_EXPORT
uc_err uc_host_ptr_auto_map_add_range(uc_engine *uc, uint64_t guest_base,
